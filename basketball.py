...
basketball_bp = Blueprint("basketball", __name__)

def get_basketball(userName):
    global insert_html
    insert_html = ""
    ...
    if len(result) == 0:
        result = "No basketball activities found"
        print(result)
        return False
...
def print_activities(result, userName):
    global insert_html
    insert_html = ""
    if result == "No basketball activities found":
        insert_html = result
    else:
...
def basketball():
    ...
    if request.method == 'POST':
        ...
        calories = getCalories.get_calories("basketball", intensity, minutes)
        if calories == -1:
            print("No weight registered")
            calories = 0
        result = add_activity(userName, "basketball", minutes, calories, water)
    ...
    <body>
        ...
        <hr>
        <h2>Your Current Basketball Activity:</h2>
        <form method="POST">
            <ol id="basketball" class="list">
                {{ actList }}
            </ol>
        </form>
        <br>
