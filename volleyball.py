...
volleyball_bp = Blueprint("volleyball", __name__)

def get_volleyball(userName):
    global insert_html
    insert_html = ""
    ...
    if len(result) == 0:
        result = "No volleyball activities found"
        print(result)
        return False
...
def print_activities(result, userName):
    global insert_html
    insert_html = ""
    if result == "No volleyball activities found":
        insert_html = result
    else:
...
def volleyball():
    ...
    if request.method == 'POST':
        ...
        calories = getCalories.get_calories("volleyball", intensity, minutes)
        if calories == -1:
            print("No weight registered")
            calories = 0
        result = add_activity(userName, "volleyball", minutes, calories, water)
    ...
    <body>
        ...
        <hr>
        <h2>Your Current Volleyball Activity:</h2>
        <form method="POST">
            <ol id="volleyball" class="list">
                {{ actList }}
            </ol>
        </form>
        <br>
