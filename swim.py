...
def get_swims(userName):
    ...
    if len(result) == 0:
        insert_html = "No swimming activities found"
        return False
...
def print_activities(result, userName):
    ...
    if result == "No swimming activities found":
        ...
        for activity in result:
            insert_html += (
                f'<li>{activity[2]}: {activity[3]} minutes, {activity[4]} calories, {activity[5]} oz of water</li>\n'
            )
    return True
...
def swim():
    ...
    if not get_swims(userName):
        print("No swimming activities found or database error.")
    ...
    <body>
        ...
        <hr>
        <h2>Your Current Swim Activity:</h2>
        <form method="POST">
