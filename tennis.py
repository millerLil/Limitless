...
def get_tennis(userName):
    ...
    if len(result) == 0:
        result = "No tennis activities found"
        print(result)
        return False
...
def print_activities(result, userName):
    global insert_html
    insert_html = ""
    if result == "No tennis activities found":
        insert_html = result
    else:
...
@tennis_bp.route('/', methods=['GET', 'POST'])
def tennis():
    ...
    if request.method == 'POST':
        ...
        calories = getCalories.get_calories("tennis", intensity, minutes)
        if calories == -1:
            print("No weight registered")
            calories = 0
        result = add_activity(userName, "tennis", minutes, calories, water)
    ...
    <body>
        ...
        <form method="POST">
            <div class="act-container">
                ...
                <div>
                    <label for="intensity">Type:</label>
                    <select type="text" name="intensity" id="intensity" required>
                        <option selected value="singles">Singles</option>
        ...
        <hr>
        <h2>Your Current Tennis Activity:</h2>
        <form method="POST">
            <ol id="tennis" class="list">
                {{ actList }}
            </ol>
        </form>
        <br>
