...
def get_yoga(userName):
    ...
    if len(result) == 0:
        result = "No yoga activities found"
        print(result)
        return False
...
def print_activities(result, userName):
    global insert_html
    insert_html = ""
    if result == "No yoga activities found":
        insert_html = result
    else:
...
def add_activity(userName, act, miles, cal, water):
    ...
    try:
        cur.execute(
            "INSERT INTO activities (userName, activity, actNum, calories, water) VALUES (?, ?, ?, ?, ?)",
...
@yoga_bp.route("/", methods=["GET", "POST"])
def yoga():
    ...
    if request.method == "POST":
        ...
        calories = getCalories.get_calories("yoga", intensity, minutes)
        if calories == -1:
            print("No weight registered")
            calories = 0
        result = add_activity(userName, "yoga", minutes, calories, water)
    ...
    <body>
        ...
        <hr>
        <h2>Your Current Yoga:</h2>
        <form method="POST">
            <ol id="yoga" class="list">
                {{ actList }}
            </ol>
        </form>
        <br><hr><br>
